...
RC BplusTreeIndex::insertNode(Block* curBlock, AttrType type, int length, char* value)
{
    ...
    for (int i = 0; i <= curBlock->tNodes; i++) {
        if (val < curBlock->value[i] && curBlock->childBlock[i] != NULL) {
            insertNode(curBlock->childBlock[i], type, length, value);
...
RC BplusTreeIndex::splitLeaf(Block* curBlock)
{
    int x, i, j;
...
RC BplusTreeIndex::splitNonLeaf(Block* curBlock)
{
    int x, i, j;
...
RC BplusTreeIndex::deleteNode(Block* curBlock, AttrType type, int length, char* value, int curBlockPosition)
{
    int* pval;
    switch (type)
    {
    ...
    for (int i = 0; dataFound == false && i <= curBlock->tNodes; i++) {
        if (val < curBlock->value[i] && curBlock->childBlock[i] != NULL) {
            deleteNode(curBlock->childBlock[i],type,length, value, i);
...
RC BplusTreeIndex::mergeBlock(Block* leftBlock, Block* rightBlock, bool isLeaf, int posOfRightBlock)
{
...
RC BplusTreeIndex::redistributeBlock(Block* leftBlock, Block* rightBlock, bool isLeaf, int posOfLeftBlock, int whichOneisCurBlock)
{
...
RC BplusTreeIndex::print(vector<Block*> Blocks)
{
